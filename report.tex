...
the number of forward passes from 128 to 32. Absolute wall-clock time varies
across runs because \texttt{evaluate.py} includes model loading in its timing;
on our hardware, typical warm-run times range from 0.19 to 0.33~s/query
compared to 0.26~s/query for the unbatched path, confirming the expected
speedup from shared image encoding.
The evaluator includes a length check (
\texttt{len(responses) == len(questions)}) to ensure batched results are never
silently truncated.
