...
\texttt{best\_speed}       & 0.5781 & --- & batched inference, no image preprocessing$^\dagger$ \\
...
Wall-clock time varies across runs (0.19--0.33~s/query typical) because
the evaluator's timer includes model loading; see text for details.}
\end{table}
...
first inference call (including one-time model loading) and stops it after the
last. Because the first call per process incurs a cold-start penalty, reported
latencies are sensitive to whether model weights are already resident in GPU
memory. For all non-batch variants we record numbers from a single
representative run on the same GPU; for \texttt{best\_speed} we omit a single
value because run-to-run variance is higher in the batched path (the model
state interacts differently with batch-level memory allocation). In
controlled back-to-back runs the batched path consistently takes fewer total
seconds than the unbatched path, as expected from 4$\times$ fewer forward
passes.
...
set. In particular, the relative frequency of colorbar-hallucination errors
